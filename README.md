...
> Requires a **rooted device** (Magisk/KernelSU recommended) and **Termux** (F-Droid build — the Play Store version won't work with root).
...
2. Find **Termux** in the list → toggle the switch to grant it root (once is enough; it stays granted).
3. Verify it works:
   ```bash
   su -c id
   ```
   You should see `uid=0(root)` in the output.
